...
local function OnEvent(self, event)
	...
	if not plugins and event == "PLAYER_ENTERING_WORLD" then
		for i = 1, GetNumAddOns() do
			...
			if enabled and find(name, "ElvUI_") and name ~= "ElvUI_" then
				plugins = plugins or {}
...
DT:RegisterDatatext("ElvUI_ Config", {"PLAYER_ENTERING_WORLD"}, OnEvent, nil, OnClick, OnEnter)