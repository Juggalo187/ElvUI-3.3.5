...
local reg = LibStub("AceConfigRegistry-3.0-ElvUI_")

local MAJOR, MINOR = "AceConfigDialog-3.0-ElvUI_", 79
...
local function GetOptionsMemberValue(membername, option, options, path, appName, ...)
	...
	if ( not allIsLiteral[membername] ) and ( type(member) == "function" or ((not stringIsLiteral[membername]) and type(member) == "string") ) then
		...
		local a, b, c, d, e, f, g, h -- ElvUI_ adds e,f,g,h for default color
		...
		if not hidden then
			if v.type == "group" then
				...
				elseif v.type == "color" then
					control = CreateControl(v.dialogControl or v.control, "ColorPicker-ElvUI_")
					control:SetLabel(name)
