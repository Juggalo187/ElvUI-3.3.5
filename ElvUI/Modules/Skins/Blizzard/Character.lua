...
S:AddCallback("Skin_Character", function()
	...
	-- Ebonhold: guarded. patch-D ships its own Interface\FrameXML\CharacterFrame.xml / PaperDollFrame.xml / PetPaperDollFrame.xml / SkillFrame.xml,
	...
	if CharacterFrameCloseButton then S:HandleCloseButton(CharacterFrameCloseButton, CharacterFrame.backdrop) end
	...
	if CharacterModelFrameRotateLeftButton then CharacterModelFrameRotateLeftButton:Point("TOPLEFT", 4, -4) end
	if CharacterModelFrameRotateRightButton then CharacterModelFrameRotateRightButton:Point("TOPLEFT", CharacterModelFrameRotateLeftButton, "TOPRIGHT", 3, 0) end
	...
	-- GearManager Dialog
	--
	-- Ebonhold: the whole equipment-manager skin is gated on the dialog even
	-- existing. patch-D ships its own Interface\FrameXML\PaperDollFrame.xml, which
	-- replaces the Blizzard original outright and does not define GearManagerDialog
	-- or any of its children -- so every line below was indexing nil. Guarding the
	-- block rather than each statement: this is one self-contained feature that is
	-- either entirely present or entirely absent, and per-line guards here were
	-- just noise that still missed the chained accesses (GearManagerDialog.backdrop
	-- :Point(), GearManagerDialogPopup.buttons[i], ...).
	if GearManagerDialog and GearManagerDialogPopup then
	if GearManagerDialog then GearManagerDialog:StripTextures() end
	if GearManagerDialog then GearManagerDialog:CreateBackdrop("Transparent") end
	...
	for _, setName in ipairs({"GearManagerDialogDeleteSet", "GearManagerDialogEquipSet", "GearManagerDialogSaveSet"}) do
		if _G[setName] then S:HandleButton(_G[setName]) end
	end

	GearSetButton1:Point("TOPLEFT", 15, -29)
	GearSetButton6:Point("TOP", GearSetButton1, "BOTTOM", 0, -13)

	if GearManagerDialogDeleteSet then GearManagerDialogDeleteSet:Point("BOTTOMLEFT", 11, 12) end
	if GearManagerDialogEquipSet then GearManagerDialogEquipSet:Point("BOTTOMLEFT", 92, 12) end
	if GearManagerDialogSaveSet then GearManagerDialogSaveSet:Point("BOTTOMRIGHT", -10, 12) end
	...
	if PetPaperDollCloseButton then S:HandleButton(PetPaperDollCloseButton) end
	...
	if PetPaperDollCloseButton then PetPaperDollCloseButton:Point("CENTER", PetPaperDollFramePetFrame, "TOPLEFT", 304, -417) end
	...
	if SkillFrameCancelButton then S:HandleButton(SkillFrameCancelButton) end
	...
	if SkillFrameCancelButton then SkillFrameCancelButton:Point("CENTER", SkillFrame, "TOPLEFT", 304, -417) end
