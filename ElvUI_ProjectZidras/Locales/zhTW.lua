...
L["FAQ_DESC"] = "本節包含一些關於 ElvUI_ 和 Project Zidras 的問題。"
L["FAQ_Elv_1"] = "|cff30ee30Q：在哪裡可以獲得 ElvUI_ 支持？|r\n|cff9482c9A：|r 最好的方法是 ElvUI_ - Backport discord - https://discord.gg/G6mUMD9TY\n要報告錯誤，您可以使用問題 跟踪器位於 https://github.com/Juggalo187/ElvUI-3.3.5/ElvUI_/issues。\n應選擇英語，但是，如果您是俄語，強烈建議您（也）用俄語編寫以更好地傳達 您給我們的俄羅斯開發人員的消息。"
...
L["FAQ_Elv_3"] = "|cff30ee30Q: 我需要提供哪些信息才能報告錯誤？|r\n|cff9482c9A:|r 首先，您需要確保錯誤確實來自 ElvUI_。\n為此，您需要 禁用除 ElvUI_ 和 ElvUI__OptionsUI 之外的所有其他插件。\n您可以通過鍵入 \"/luaerror on\"（不帶引號）來執行此操作。\n如果錯誤沒有消失，那麼您應該向我們發送錯誤報告。\n在報告中 ，您需要提供 ElvUI_ 版本（\"latest\" 不是有效的版本號），錯誤文本，可以截圖。\n您提供的有關如何重現錯誤的信息越多，速度越快 是為了可能把它修好。"
L["FAQ_Elv_4"] = "|cff30ee30Q: 為什麼有些選項在使用相同配置文件時沒有應用到其他字符上？|r\n|cff9482c9A:|r ElvUI_ 有三種選項。首先（配置文件）存儲在您的 配置文件，第二個（私有）以字符為基礎存儲，第三個（全局）應用於所有字符，而不管使用的配置文件如何。\n在這種情況下，您很可能遇到了第二類選項，即私有的，這些選項基於每個 特點。"
L["FAQ_Elv_5"] = "|cff30ee30Q: ElvUI_ 斜線（聊天）命令是什麼？|r\n|cff9482c9A:|r ElvUI_ 有很多不同的聊天命令用於不同的目的。它們是：\n/ec 或/ElvUI_ - 打開配置窗口\n/bgstats - 如果你在戰場上並關閉那些，則顯示戰場特定的數據文本\n/hellokitty - 想要粉紅色的卡哇伊用戶界面？我們滿足你！\n/harlemshake - 需要搖一搖嗎？就做它！\n/luaerror - 在測試模式下加載您的 UI，該模式旨在製作正確的錯誤報告（請參閱 Q #3）\n/egrid - 在切換錨點模式下設置網格的大小\n/moveui - 允許移動東西\n/resetui - 重置整個用戶界面"
L["FAQ_pz_1"] = "|cff30ee30Q: 如果在 Project Zidras 中遇到錯誤，我該怎麼辦？|r\n|cff9482c9A:|r 與 ElvUI_ 幾乎相同（請參閱其常見問題部分），但您還必須在以下位置提供 PZ 插件版本：https://github.com/Zidras/ElvUI_ProjectZidras/issues。"
L["FAQ_pz_2"] = "|cff30ee30Q: Project Zidras 是否與 ElvUI_ 具有相同的語言政策？|r\n|cff9482c9A:|r 是的，英語應是唯一選擇的語言。"
...
L["HD-Nameplates_DESC"] = "將 ElvUI_ castbars 應用於所有單位的默認暴雪銘牌 castbars 和緩存的 GUID 銘牌。"
...
L["Modules"] = "模塊"
L["Normal"] = "普通"
L["Options for customizing unit frames. Please don't change these setting when ElvUI_'s testing frames for bosses and arena teams are shown. That will make them invisible until retoggling."] = "自定義單位框架的選項。 請不要在顯示 ElvUI_ 的 Boss 和競技場團隊測試框架時更改這些設置。 這將使它們不可見，直到重新切換。"
L["Over Absorbs"] = "滿血吸收"
L["Over Heal Absorbs"] = "過量治療吸收"
L["Overflow"] = "溢出"
L["Party"] = "隊伍"
L["Plugin for |cff1784d1ElvUI|r by Zidras."] = "插件 |cff1784d1ElvUI|r 由 Zidras 提供。"
L["PZ_DESC"] = "|cff00BFFFProject Zidras|r 是 ElvUI_ 的擴展。 它增加了：\n- 對銘牌進行了大量改進，專門針對 HD 客戶端\n- 聊天：添加了組角色和 GM 圖標\n- Unitframes：添加了吸收預測欄； 添加角色圖標\n-（更多內容......）\n\n|cff3cbf27注意：|r 基於零售 Shadow & Light 的插件結構。"
