...
local moveRetries = 0
local moveTracker = {}
local lastItemID, lockStop, lastDestination, lastMove, lastDynamicWait
...
local function GetDynamicWaitTime(isGuildBank)
    local _, _, latency = GetNetStats()
    -- Base times: faster for normal bags, slightly slower for guild bank
    local base = isGuildBank and 0.03 or 0.013
    local latencyFactor = latency / 2500
    local totalWait = base + latencyFactor
    
    -- Tighter bounds: min 0.05s (faster), max 0.3s
    totalWait = math.max(0.05, math.min(0.3, totalWait))
    
    return totalWait
end

-- Initialize with dynamic wait time
local newWait = GetDynamicWaitTime(false) or 0.05
WAIT_TIME = newWait

local newWait = lastDynamicWait or GetDynamicWaitTime(false) or 0.1
WAIT_TIME = newWait
do
	local t = 0
...
function B.Sort(bags, sorter, invertDirection)
	...
	for i, bag, slot in B:IterateBags(bags, nil, "both") do
		...
		if link then
			...
			if not blackListedSlots[bagSlot] and #blackListQueries > 0 then
				for _, itemsearchquery in ipairs(blackListQueries) do
	...
	while passNeeded do
		passNeeded = false
		local i = 1
		for _, bag, slot in B:IterateBags(bags, invertDirection, "both") do
...
function B:DoMove(move)
    ...
    -- Return dynamic wait time
    local wasGuild = sourceGuild or targetGuild
    local dynamicWait = GetDynamicWaitTime(wasGuild)
    
    return true, sourceItemID, source, targetItemID, target, wasGuild, dynamicWait
end
...
function B:DoMoves()
    ...
    if cursorType == "item" and cursorItemID then
        ...
        if moveRetries < 100 then
            ...
            if not targetLocked then
                B:PickupItem(targetBag, targetSlot)
                WAIT_TIME = lastDynamicWait or GetDynamicWaitTime(false) or 0.05
                lockStop = GetTime()
            ...
            if actualItemID ~= itemID then
                ...
                if (GetTime() - lockStop) > MAX_MOVE_TIME then
                    if lastMove and moveRetries < 100 then
                        local success, moveID, moveSource, targetID, moveTarget, wasGuild, dynamicWait = B:DoMove(lastMove)
                        lastDynamicWait = dynamicWait
                        WAIT_TIME = dynamicWait or 0.05
    ...
    lastItemID, lockStop, lastDestination, lastMove, lastDynamicWait = nil, nil, nil, nil, nil
    wipe(moveTracker)

    local success, moveID, targetID, moveSource, moveTarget, wasGuild, dynamicWait
    if #moves > 0 then
        for i = #moves, 1, -1 do
            success, moveID, moveSource, targetID, moveTarget, wasGuild, dynamicWait = B:DoMove(moves[i])
            if not success then
                lastDynamicWait = dynamicWait
                WAIT_TIME = dynamicWait or 0.05
                lockStop = GetTime()
...
function B:ShouldShowSpinner()
    local count = 0
    for _, bag, slot in B:IterateBags(allBags) do
        if B:GetItemID(bag, slot) then
            count = count + 1
            if count > 50 then  -- Only show spinner for 50+ items
                return true
            end
        end
    end
