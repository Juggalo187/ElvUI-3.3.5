...
do
	...
	AddOn:AddLib("ACL", "AceLocale-3.0-ElvUI_")
	AddOn:AddLib("LAB", "LibActionButton-1.0-ElvUI_")
	AddOn:AddLib("LAI", "LibAuraInfo-1.0-ElvUI_", true)
	...
	AddOn:AddLib("ItemSearch", "LibItemSearch-1.2-ElvUI_")
	AddOn:AddLib("Compress", "LibCompress")
	AddOn:AddLib("Base64", "LibBase64-1.0-ElvUI_")
	AddOn:AddLib("Translit", "LibTranslit-1.0")
	-- added on ElvUI__OptionsUI load: AceGUI, AceConfig, AceConfigDialog, AceConfigRegistry, AceDBOptions
...
function AddOn:ToggleOptionsUI(msg)
	...
	if not IsAddOnLoaded("ElvUI__OptionsUI") then
		local noConfig
		local _, _, _, _, reason = GetAddOnInfo("ElvUI__OptionsUI")
		if reason ~= "MISSING" and reason ~= "DISABLED" then
			self.GUIFrame = false
			LoadAddOn("ElvUI__OptionsUI")
			...
			if not IsAddOnLoaded("ElvUI__OptionsUI") then noConfig = true end

			-- version check ElvUI_ options if it's actually enabled
			if (not noConfig) and GetAddOnMetadata("ElvUI__OptionsUI", "Version") ~= "1.06" then
		...
		if noConfig then
			self:Print("|cffff0000Error -- Addon 'ElvUI__OptionsUI' not found or is disabled.|r")
			return
		end
	end
		...
		if pages ~= nil then
			...
			if pageCount > 1 then
				...
				for i = 1, pageCount do
					if i == 1 then
						main = pages[i] and ACD and ACD.Status and ACD.Status.ElvUI_
				...
				local main = pages[1] and ACD and ACD.Status and ACD.Status.ElvUI_
				mainSel = main and main.status and main.status.groups and main.status.groups.selected
			end
