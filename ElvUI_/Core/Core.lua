...
E.version = GetAddOnMetadata("ElvUI_", "Version")
...
--This frame everything in ElvUI_ should be anchored to for Eyefinity support.
...
function E:Print(...)
	(_G[self.db.general.messageRedirect] or DEFAULT_CHAT_FRAME):AddMessage(strjoin(" ", self:ColorizedName("ElvUI_", true), ...)) -- I put DEFAULT_CHAT_FRAME as a fail safe.
end
...
function E:GrabColorPickerValues(r, g, b)
	-- we must block the execution path to `ColorCallback` in `AceGUIWidget-ColorPicker-ElvUI_`
...
function E:UpdateMedia()
	...
	-- Textures
	self.media.blankTex = LSM:Fetch("background", "ElvUI_ Blank")
...
do	--Update font/texture paths when they are registered by the addon providing them
	--This helps fix most of the issues with fonts or textures reverting to default because the addon providing them is loading after ElvUI_.
	--We use a wrapper to avoid errors in :UpdateMedia because "self" is passed to the function with a value other than ElvUI_.
...
function E:IncompatibleAddOn(addon, module)
	...
	E.PopupDialogs.INCOMPATIBLE_ADDON.button2 = "ElvUI_ "..module
...
function E:Initialize()
	...
	E.Libs.DualSpec:EnhanceDatabase(self.data, "ElvUI_")
