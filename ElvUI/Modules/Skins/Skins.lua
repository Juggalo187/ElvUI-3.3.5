...
-- Ebonhold: every S:Handle* below now returns quietly when handed a nil
-- widget instead of indexing it.
--
-- WHY: patch-D ships its own Interface\FrameXML\* (34 files), and each one
-- fully REPLACES the Blizzard original -- so any widget the stock file defined
-- and the custom one does not is simply absent at runtime. The skins call these
-- handlers with those globals directly (S:HandleRotateButton(CharacterModel
-- FrameRotateLeftButton), S:HandleCloseButton(SpellBookCloseButton), ...), so a
-- dropped widget threw 'attempt to index local btn (a nil value)' -- and since
-- each Skin_* is a single callback, everything after the throw was skipped and
-- the rest of that panel stayed unskinned.
--
-- Guarding here rather than at the call sites is deliberate: there are hundreds
-- of call sites and the FrameXML overrides keep moving, so a per-site fix is an
-- endless game of whack-a-mole. A missing widget now costs exactly one
-- unskinned element instead of an aborted skin.
...
function S:HandleButtonHighlight(frame, r, g, b, a)
	if not frame then return end	-- see the note above S:HandleButton
...
function S:HandleTab(tab, noBackdrop)
	if not tab then return end	-- see the note above S:HandleButton
...
function S:HandleRotateButton(btn)
	if not btn then return end	-- see the note above S:HandleButton
...
function S:HandleIcon(icon, parent)
	if not icon then return end	-- see the note above S:HandleButton
...
function S:HandleItemButton(b, shrinkIcon)
	if not b then return end	-- see the note above S:HandleButton
...
function S:HandleCloseButton(f, point)
	if not f then return end	-- see the note above S:HandleButton
	f:StripTextures()
...
function S:HandleCollapseExpandButton(button, defaultState, useFontString, xOffset, yOffset)
	if not button then return end	-- see the note above S:HandleButton
	if button.isSkinned then return end
