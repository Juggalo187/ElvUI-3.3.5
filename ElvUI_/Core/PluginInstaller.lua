...
--[[
	...
	Title is wat displayed on top of the window. By default it's ""ElvUI_ Plugin Installation""
	Name is how your installation will be showin in "pending list", Default is "Unknown"
	tutorialImage is a path to your own texture to use in frame. if not specified, then it will use ElvUI_'s one
...
function PI:RunInstall()
	...
	if self.Installs[1] and not PluginInstallFrame:IsShown() and not (ElvUIInstallFrame and ElvUIInstallFrame:IsShown()) then
		...
		f.Title:SetText(db.Title or L["ElvUI_ Plugin Installation"])
