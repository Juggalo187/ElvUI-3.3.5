...
function NP:OnNamePlateUnitRemoved(unitToken)
    ...
    -- The native plate's OnHide fires when the unit leaves, and ElvUI_ cleans up.
...
function NP:UpdateCVars()
	...
	if GetCVar("nameplateOriginPos") ~= nil and string.find(string.lower(E.myrealm), "frostmourne", 1, true) then
		local offset = self.db and self.db.customOffset or -20
		SetCVar("nameplateOriginPos", tostring(offset))
	end
end
...
function NP:PLAYER_ENTERING_WORLD()
	...
	if GetCVar("nameplateOriginPos") ~= nil and string.find(string.lower(E.myrealm), "frostmourne", 1, true) then
		-- Wait 1 second for the client engine to finish loading world defaults
		E:Delay(1, function()
			if NP.db and NP.db.customOffset then
				SetCVar("nameplateOriginPos", tostring(NP.db.customOffset))
			end
		end)
	end
end
