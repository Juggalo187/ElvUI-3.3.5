...
local StripString = E.StripString or function(s)
	-- crude fallback: remove color codes and escape sequences
	return (gsub(s or '', '|c%x%x%x%x%x%x%x%x|r', ''))
end
...
local EditBox = ACH:Input(L["Search"], nil, 0, nil, 1, function() return SearchText end, function(_, value) C:Search_ClearResults() if strmatch(value, '%S+') then SearchText = strtrim(strlower(value)) C:Search_Config() C:Search_AddResults() end end)
...
function C:Search_ButtonFunc()
	if self.option then
		E.Libs.AceConfigDialog:SelectGroup('ElvUI_', strsplit(',', self.option.location))
	end
end
...
function C:Search_FindText(text, whatsNew)
	if whatsNew then
		...
		return strfind(strlower(StripString(text)), SearchText, nil, true)
	end
end
